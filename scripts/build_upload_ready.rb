...
flags = {
  :base => '/var/geohydra/current/upload',
...
def build(shp, druid, geometryType, flags)
  ...
  Dir.glob("#{readydir}/**/#{basename}.*") do |fn|
    FileUtils.ln_sf([File.expand_path(fn)], tempdir, :verbose => true)
  end
...
CSV.foreach('/var/geohydra/current/upload/metadata/Batch20130806.csv') do |row|
